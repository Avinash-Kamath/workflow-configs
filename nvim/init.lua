...
-- Prevent netrw from loading (neo-tree replaces it)
...
-- Disable swap files (prevents E325 conflicts with auto-session)
vim.o.swapfile = false

-- Folding is configured by nvim-ufo (custom/plugins/ufo.lua)
...
-- Faster scrolling
vim.keymap.set('n', '<C-d>', '10jzz', { desc = 'Scroll down fast' })
vim.keymap.set('n', '<C-u>', '10kzz', { desc = 'Scroll up fast' })

-- Swap jumplist navigation (C-o = forward, C-i = back)
vim.keymap.set('n', '<C-o>', '<C-i>', { desc = 'Jump forward in jumplist' })
vim.keymap.set('n', '<C-i>', '<C-o>', { desc = 'Jump backward in jumplist' })
...
require('lazy').setup({
  ...
  { -- Fuzzy Finder (files, lsp, etc)
    ...
    config = function()
      ...
      vim.keymap.set('n', '<leader>sm', function() builtin.find_files { cwd = vim.fn.expand '%:h' } end, { desc = '[S]earch files in current [M]odule dir' })
    end,
  },
  ...
  { -- Collection of various small independent plugins/modules
    ...
    config = function()
      ...
      MiniStatusline.active = function()
        ...
        local diagnostics = ''
        if errors > 0 then diagnostics = diagnostics .. '%#DiagnosticError# E:' .. errors .. ' %*' end
        if warnings > 0 then diagnostics = diagnostics .. '%#DiagnosticWarn# W:' .. warnings .. ' %*' end
